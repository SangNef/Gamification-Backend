...
@section('content')
    <section class="bg-gray-200 min-h-screen w-full pl-6 sm:pl-[88px] pr-6 overflow-hidden font-inter">
        <div class="h-4 flex items-center gap-1 my-2">
            ...
            <p class="text-gray-400 mt-3">/ Package manage</p>
        </div>
        <h1 class="font-bold text-2xl leading-7 my-6">Package manage</h1>
        @if (session('success'))
        ...
        @endif
        <div class="mt-4">
            <a href="/admin/package-manage/create" class="bg-pink-400 text-white py-2 px-4 rounded no-underline">Add New
                Package</a>
        </div>
        <div class="w-full bg-white rounded-lg shadow my-4 p-6">
            <h2 class="text-xl leading-7 font-bold">Package list</h2>
                            ...
                            <td class="p-2 border-b text-start">
                                ...
                                <form action="{{ route('package.delete', $package->stt) }}" method="POST" class="inline"
                                    ...
                                    <button type="button" class="text-red-500 ml-2 deleteButton"
                                        data-packagename="{{ $package->name }}"><i
        ...
        <div class="w-full bg-white rounded-lg shadow my-4 p-6">
            <h2 class="text-xl leading-7 font-bold">Orders</h2>
            <table class="w-full border-gray-300">
                <thead>
                    <tr class="bg-gray-200 text-xs leading-4 font-medium tracking-wider uppercase text-gray-500">
                        <th class="p-2 border-b text-start">Id</th>
                        <th class="p-2 border-b text-start">User</th>
                        <th class="p-2 border-b text-start">Package</th>
                        <th class="p-2 border-b text-start">Price</th>
                ...
                <tbody>
                    @foreach ($orders as $item)
                        <tr
                            ...
                            <td class="p-2 border-b">{{ $item->package_point }}</td>
                            <td class="p-2 border-b">{{ $item->price }}</td>
            ...
            <nav class="mt-4">
                <ul class="pagination justify-content-center">
                    {{ $orders->onEachSide(2)->appends(Request::query())->links('pagination::bootstrap-4') }}
                </ul>
            </nav>
        </div>
    </section>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(function(button) {
                button.addEventListener('click', function() {
                    // Get the package name from the data-packagename attribute
                    var packageName = button.getAttribute('data-packagename');

                    // Display SweetAlert2 confirmation popup
                    Swal.fire({
                        title: 'Are you sure?',
                        text: 'Do you want to delete ' + packageName + ' package?',
                        icon: 'warning',
