...
@section('content')
    <section class="bg-gray-200 min-h-screen w-full pl-6 sm:pl-[88px] pr-6 overflow-hidden font-inter">
        <div class="h-4 flex items-center gap-1 my-2">
            ...
            <p class="text-gray-400 mt-3">/ Enemies manage</p>
        </div>
        <h1 class="font-bold text-2xl leading-7 my-6">Enemies manage</h1>
        @if (session('success'))
        ...
        @endif
        <div class="mt-4">
            <a href="/admin/enemy-manage/create" class="bg-pink-400 text-white py-2 px-4 rounded no-underline">Add New Enemy</a>
        </div>
        <div class="w-full bg-white rounded-lg shadow my-4 p-6">
            <h2 class="text-xl leading-7 font-bold">Enemies list</h2>
                ...
                <tbody>
                    @foreach ($enemies as $enemy)
                        <tr
                            ...
                            <td class="p-2 border-b">{{ $enemy->id }}</td>
                            <td class="p-2 border-b">{{ $enemy->name }}</td>
                            <td class="p-2 border-b">{{ $enemy->type }}</td>
                            <td class="p-2 border-b">{{ $enemy->rank }}</td>
                            <td class="p-2 border-b text-start">
                                <a href="/admin/enemy-manage/enemy-{{ $enemy->id }}-detail" class="text-blue-500"><i
                                    class="fa-regular fa-eye"></i></a>
                            <a href="/admin/enemy-manage/enemy-{{ $enemy->id }}-update" class="text-yellow-500 ml-2"><i
                                    class="fa-regular fa-pen-to-square"></i></a>
                            <!-- Modify your delete button code -->
                            <form action="{{ route('enemy.delete', $enemy->id) }}" method="POST" class="inline"
                                id="deleteForm{{ $enemy->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="text-red-500 ml-2 deleteButton"
                                    data-itemname="{{ $enemy->name }}"><i
