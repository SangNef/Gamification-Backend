...
@section('content')
    <section class="bg-gray-200 min-h-screen w-full pl-6 sm:pl-[88px] pr-6 overflow-hidden font-inter">
        <div class="h-4 flex items-center gap-1 my-2">
            ...
            <p class="text-gray-400 mt-3">/ Quest manage</p>
        </div>
        <h1 class="font-bold text-2xl leading-7 my-6">Quest manage</h1>
        @if (session('success'))
        ...
        @endif
        <div class="mt-4">
            <a href="/admin/quest-manage/create" class="bg-pink-400 text-white py-2 px-4 rounded no-underline">Add New Quest</a>
        </div>
        <div class="w-full bg-white rounded-lg shadow my-4 p-6">
            <h2 class="text-xl leading-7 font-bold">Quest list</h2>
            <table class="w-full border-gray-300">
                <thead>
                    <tr class="bg-gray-200 text-xs leading-4 font-medium tracking-wider uppercase text-gray-500">
                        ...
                        <th class="p-2 border-b text-start">Max completion</th>
                ...
                <tbody>
                    @foreach ($quests as $quest)
                        <tr
                            ...
                            <td class="p-2 border-b">{{ $quest->id }}</td>
                            <td class="p-2 border-b">{{ $quest->name }}</td>
                            <td class="p-2 border-b">{{ $quest->type }}</td>
                            ...
                            <td class="p-2 border-b text-start">
                                ...
                                <form action="{{ route('quest.delete', $quest->id) }}" method="POST" class="inline"
                                    id="deleteForm{{ $quest->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="text-red-500 ml-2 deleteButton"
                                        data-packagename="{{ $quest->name }}"><i
        ...
        <div class="w-full bg-white rounded-lg shadow my-4 p-6">
            <h2 class="text-xl leading-7 font-bold">Quest progess</h2>
            <table class="w-full border-gray-300">
                <thead>
                    <tr class="bg-gray-200 text-xs leading-4 font-medium tracking-wider uppercase text-gray-500">
                        <th class="p-2 border-b text-start">Id</th>
                        <th class="p-2 border-b text-start">User</th>
                        <th class="p-2 border-b text-start">Quest</th>
                        <th class="p-2 border-b text-start">Point</th>
                        <th class="p-2 border-b text-start">Buy at</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($progess as $item)
                        <tr
                            ...
                            <td class="p-2 border-b">{{ $item->quest_name }}</td>
                            <td class="p-2 border-b">{{ $item->point }}</td>
            ...
            <nav class="mt-4">
                <ul class="pagination justify-content-center">
                    {{ $progess->onEachSide(2)->appends(Request::query())->links('pagination::bootstrap-4') }}
                </ul>
            </nav>
        </div>
    </section>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(function(button) {
                button.addEventListener('click', function() {
                    // Get the quest name from the data-packagename attribute
                    var packageName = button.getAttribute('data-packagename');

                    // Display SweetAlert2 confirmation popup
                    Swal.fire({
                        title: 'Are you sure?',
                        text: 'Do you want to delete ' + packageName + ' quest?',
                        icon: 'warning',
