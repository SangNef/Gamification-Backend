...
@section('content')
    <section class="bg-gray-200 min-h-screen w-full pl-6 sm:pl-[88px] pr-6 overflow-hidden font-inter">
        <div class="h-4 flex items-center gap-1 my-2">
            ...
            <p class="text-gray-400 mt-3">/ Event manage</p>
        </div>
        <h1 class="font-bold text-2xl leading-7 my-6">Event manage</h1>
        @if (session('success'))
        ...
        @endif
        <div class="mt-4">
            <a href="/admin/event-manage/create" class="bg-pink-400 text-white py-2 px-4 rounded no-underline">Add New Event</a>
        </div>
        <div class="w-full bg-white rounded-lg shadow my-4 p-6">
            <h2 class="text-xl leading-7 font-bold">Event list</h2>
            <table class="w-full border-gray-300">
                <thead>
                    <tr class="bg-gray-200 text-xs leading-4 font-medium tracking-wider uppercase text-gray-500">
                        ...
                        <th class="p-2 border-b text-start">Title</th>
                        <th class="p-2 border-b text-start">Start date</th>
                        <th class="p-2 border-b text-start">End date</th>
                        <th class="p-2 border-b text-start">Items</th>
                ...
                <tbody>
                    @foreach ($events as $event)
                        <tr
                            ...
                            <td class="p-2 border-b">{{ $event->start }}</td>
                            <td class="p-2 border-b">{{ $event->end }}</td>
