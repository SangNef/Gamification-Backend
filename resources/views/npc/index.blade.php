...
<section class="bg-gray-200 min-h-screen w-full pl-6 sm:pl-[88px] pr-6 overflow-hidden font-inter">
    <div class="h-4 flex items-center gap-1 my-2">
        ...
        <p class="text-gray-400 mt-3">/ NPC manage</p>
    </div>
    <h1 class="font-bold text-2xl leading-7 my-6">NPC manage</h1>
    @if (session('success'))
    ...
    @endif
    <div class="mt-4">
        <a href="/admin/npc-manage/create"
            class="bg-pink-400 text-white py-2 px-4 rounded no-underline">Add New NPC</a>
    </div>
    <div class="w-full bg-white rounded-lg shadow my-4 p-6">
        <h2 class="text-xl leading-7 font-bold">NPC list</h2>
        <table class="w-full border-gray-300">
            <thead>
                <tr class="bg-gray-200 text-xs leading-4 font-medium tracking-wider uppercase text-gray-500">
                    ...
                    <th class="p-2 border-b text-start">Access</th>
                    <th class="p-2 border-b text-start">Role</th>
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-100 even:bg-gray-200 duration-150 text-sm leading-5 font-normal text-gray-500">
                    <td class="p-2 border-b">{{ $npc->id }}</td>
                    <td class="p-2 border-b">{{ $npc->name }}</td>
                    <td class="p-2 border-b">{{ $npc->access }}</td>
                    <td class="p-2 border-b">{{ $npc->role }}</td>
                    <td class="p-2 border-b text-start">
                        <a href="/admin/npc-manage/npc-{{ $npc->id }}-detail" class="text-blue-500"><i
                                class="fa-regular fa-eye"></i></a>
                        <a href="/admin/npc-manage/update-{{ $npc->id }}" class="text-yellow-500 ml-2"><i
                                class="fa-regular fa-pen-to-square"></i></a>
                        <!-- Modify your delete button code -->
                        <form action="{{ route('npc.delete', $npc->id) }}" method="POST" class="inline"
                            id="deleteForm{{ $npc->id }}">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="text-red-500 ml-2 deleteButton"
                                data-npcname="{{ $npc->name }}"><i class="fa-regular fa-trash-can"></i></button>
                        </form>
                    </td>
                </tr>
                @endforeach
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                // Get the npc name from the data-npcname attribute
                var npcName = button.getAttribute('data-npcname');

                // Display SweetAlert2 confirmation popup
                Swal.fire({
                    title: 'Are you sure?',
                    text: 'Do you want to delete ' + npcName + ' NPC?',
                    icon: 'warning',
